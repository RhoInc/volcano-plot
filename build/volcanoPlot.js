...
function setDefaults(settings) {
    ...
    settings.color_col = settings.color_col ? settings.color_col : settings.structure_cols.length >= 1 ? settings.structure_cols[0].value_col || settings.structure_cols[0] : defaultSettings.color_col;
...
function drawSelected(data) {
    ...
    var tables = this,
        rows = this.selected.table.select('tbody').selectAll('tr.selected').data(data.filter(function (d, i) {
        return i < 25 * _this.selected.multiplier;
    })).enter().append('tr').classed('selected', true).on('click', function (d) {
...
function drawDetails(datum) {
    ...
    if (datum) {
        ...
        this.details.data.stats = this.parent.data.clean.splice(5).filter(function (d) {
            return d[settings.id_col.value_col || settings.id_col] == datum[settings.id_col.value_col || settings.id_col];
        });
